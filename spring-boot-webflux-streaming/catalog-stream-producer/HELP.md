...
This service accepts a list of catalog codes and streams back data for each code in chunks.

Instead of returning one large response, it breaks the data into smaller pieces and sends them progressively. This helps when dealing with large datasets and avoids waiting for the full response.
...
curl.exe -N -X POST "http://localhost:8081/catalog/chunks/stream" `
